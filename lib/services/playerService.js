 ...
 */
export async function searchPlayers(filters = {}, token) {
  if (!token) {
 ...
 */
export async function fetchAvailableComuni(token) {
  if (!token) {
    return { success: false, error: 'Token mancante' }
  }
 ...
 */
export async function updateUserAvailability(available, token) {
  if (!token) {
    return { success: false, error: 'Token mancante' }
  }
